...
sudo docker compose build seekall-api 2>&1 | tail -10

echo ""
echo "=== 4. docker compose up seekall-api ==="
sudo docker compose up -d --no-deps seekall-api 2>&1 | tail -5

echo ""
echo "=== 5. wait 25s + verify ==="
sleep 25
echo -n "container: "
sudo docker ps --filter "name=seekall-api" --format "{{.Names}} {{.Status}}"
echo -n "health: "
curl -sS http://172.18.0.5:7301/api/v1/health
echo

echo ""
echo "=== 6. Swagger 收敛验证 (期望 /docs 404) ==="
echo -n "/docs: "
curl -sS -o /dev/null -w "HTTP %{http_code}\n" http://172.18.0.5:7301/docs
echo -n "/docs-json: "
curl -sS -o /dev/null -w "HTTP %{http_code}\n" http://172.18.0.5:7301/docs-json

echo ""
echo "=== 7. 限流验证 (login 6 次连击,期望第 6 次 429) ==="
for i in 1 2 3 4 5 6; do
  CODE=$(curl -sS -o /dev/null -w "%{http_code}" -X POST -H "Content-Type: application/json" \
    http://172.18.0.5:7301/api/v1/auth/login \
