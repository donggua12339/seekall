...
| v0.7   | 待定          | 搜索追平竞品                        | ⏳ 进行中                             |
| v0.8   | 取消          | 社区生态 + 商业化                   | ❌ 已取消（2026-08-05，无用户前不做） |
| v1.0   | 待定          | 稳定版                              | 📋 规划中                             |
...
- [x] 搜索结果加时间范围筛选（2026-08-05：资源库维度实现——/resources/hot+latest 支持 range=today/week/month/year + category 过滤，基于索引 firstSeenTs；各源原始结果仍无发布时间戳）
...
## v0.8 — 社区生态 + 商业化（❌ 已取消，2026-08-05）

> 决策：当前 0 用户，做积分/等级/分成等社区商业化是空转。先拉新，有真实用户后再评估是否重启。
...
## 当前阶段 — 拉新获客（v0.7 收尾后最高优先）

> 产品功能已够用（v0.7 P1+P2 完成），瓶颈在"没人知道这个站"。停止加功能，转向让人来。

- [ ] SEO 落地：GSC 提交 sitemap + Baidu 站长提交（此前遗留的手动项）
- [ ] 内容分发：docs/blog + 小红书草稿 + content-publish-handoff 交接内容发出去
- [ ] 冷启动渠道：目标社区（软件/资源类论坛、贴吧、TG 群）铺入口
- [ ] 转化观测：确认搜索/注册漏斗有基础埋点，能看到来的人留没留下
